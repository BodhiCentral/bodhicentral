...
Reference material for working with the PostHog analytics integration already installed in this project (see `instrumentation-client.ts`, `src/lib/posthog-server.ts`, and the `/ingest` reverse proxy in `next.config.mjs`). Consult this when adding new events, touching auth/identify flows, or debugging capture issues.

## Reference files

- `references/EXAMPLE.md` - Next.js App Router example project code showing the target implementation pattern (instrumentation-client.ts init, identify, event capture, server-side tracking, reverse proxy config)
- `references/next-js.md` - PostHog's Next.js integration docs
- `references/identify-users.md` - Identify users docs

Consult the documentation for API details; match the example project's patterns when adding new integration points.
